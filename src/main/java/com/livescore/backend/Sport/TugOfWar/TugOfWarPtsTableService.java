...
// Tug of War: Win=2pts (same as badminton/TT)
@Service
@RequiredArgsConstructor
public class TugOfWarPtsTableService {
    ...
    public void updateAfterMatch(Long matchId) {
        ...
        if (t1Won) {
            pts1.setWins(safe(pts1.getWins()) + 1);   pts1.setPoints(safe(pts1.getPoints()) + 2);
            pts2.setLosses(safe(pts2.getLosses()) + 1);
        } else {
            pts2.setWins(safe(pts2.getWins()) + 1);   pts2.setPoints(safe(pts2.getPoints()) + 2);
    ...
    public ResponseEntity<?> getTable(Long tournamentId) {
        ...
        List<PtsTableDTO> dtos = tables.stream().map(pt -> {
            ...
            dto.setLosses(safe(pt.getLosses())); dto.setPoints(safe(pt.getPoints()));
            return dto;
    ...
    private PtsTable getOrCreate(Long tid, Team team, Tournament tournament) {
        ...
        if (e != null) return e;
        PtsTable p = new PtsTable();
        p.setTeam(team); p.setTournament(tournament);
