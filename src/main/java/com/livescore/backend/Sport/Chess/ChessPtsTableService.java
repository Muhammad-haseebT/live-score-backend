 ...
 *   Loss = 0 pts
 *
 * goalsDifference used as tiebreaker (checks = assists)
 */
@Service
@RequiredArgsConstructor
public class ChessPtsTableService {
    ...
    public void updateAfterMatch(Long matchId) {
        ...
        boolean isDraw = match.getWinnerTeam() == null;
        ...
        if (isDraw) {
            ...
            if (t1Won) {
                pts1.setWins(safe(pts1.getWins()) + 1);   pts1.setPoints(safe(pts1.getPoints()) + 3);
                pts2.setLosses(safe(pts2.getLosses()) + 1);
            } else {
                pts2.setWins(safe(pts2.getWins()) + 1);   pts2.setPoints(safe(pts2.getPoints()) + 3);
    ...
    public ResponseEntity<?> getTable(Long tournamentId) {
        ...
        List<PtsTableDTO> dtos = tables.stream().map(pt -> {
            ...
            dto.setId(pt.getId()); dto.setTeamId(pt.getTeam().getId());
            dto.setTeamName(pt.getTeam().getName()); dto.setTournamentId(tournamentId);
            dto.setPlayed(safe(pt.getPlayed())); dto.setWins(safe(pt.getWins()));
            dto.setLosses(safe(pt.getLosses())); dto.setDraws(safe(pt.getDraws()));
    ...
    private PtsTable getOrCreate(Long tid, Team team, Tournament tournament) {
        ...
        p.setPlayed(0); p.setWins(0); p.setLosses(0); p.setDraws(0); p.setPoints(0);
