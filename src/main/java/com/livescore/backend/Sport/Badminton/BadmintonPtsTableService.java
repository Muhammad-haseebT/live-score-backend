 ...
 *   Match Win  = 2 pts
 *   Match Loss = 0 pts
 *   goalsFor/Against = games (sets) won/lost (for tiebreaking)
 */
@Service
@RequiredArgsConstructor
public class BadmintonPtsTableService {
    ...
    public void updateAfterMatch(Long matchId) {
        ...
        if (t1Won) {
            ...
            pts1.setPoints(safe(pts1.getPoints()) + 2);
        } else {
            pts2.setWins(safe(pts2.getWins()) + 1);
            pts1.setLosses(safe(pts1.getLosses()) + 1);
            pts2.setPoints(safe(pts2.getPoints()) + 2);
        }
