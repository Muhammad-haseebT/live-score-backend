...
/**
 * Stats mapping (reuses Stats entity):
 *   goals       = total points scored
 *   assists     = smashes + service aces (attacking shots)
 *   fouls       = faults committed (net/foot)
 *   yellowCards = "out" shots (forced out)
 *   redCards    = 0 (unused)
 */
@Service
@RequiredArgsConstructor
public class BadmintonStatsService {
    ...
    @Transactional
    public void onEventSaved(BadmintonEvent event) {
        if (event == null || event.getMatch() == null
                || event.getMatch().getTournament() == null) return;
    ...
    public void onMatchEnd(Long matchId) {
        ...
        Set<Long> pids = badmintonEventInterface.findByMatch_IdOrderByIdAsc(matchId)
    ...
    public void recalculatePlayerStats(Long playerId, Long tournamentId) {
        ...
        int pom = awardInterface.countPomByPlayerIdAndSport(playerId, "badminton");
    ...
    private void calculatePOM(Long matchId, Match match) {
        ...
        badmintonEventInterface.findByMatch_IdOrderByIdAsc(matchId).forEach(ev -> {
    ...
    public void updateTournamentAwards(Tournament tournament) {
        ...
        all.stream().filter(s -> s.getAssists() != null && s.getAssists() > 0)
                ...
                .ifPresent(s -> saveAward(null, tournament, s.getPlayer(),
                        "TOP_ATTACKER", s.getAssists(), "Most smashes/aces: " + s.getAssists()));
    ...
    private void ensureStats(Long playerId, Long tournamentId, Tournament tournament) {
        ...
        if (p == null) return;
        Stats s = new Stats();
        s.setPlayer(p); s.setTournament(tournament); s.setSportType(tournament.getSport());
    ...
    private void saveAward(Match m, Tournament t, Player p, String type, int pts, String reason) {
        Award a = new Award();
        a.setMatch(m); a.setTournament(t); a.setPlayer(p);
